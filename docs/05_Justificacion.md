# ms-rutinas — Justificación del servicio y cobertura de requisitos

**Caso caso15 — FitLife** (una plataforma de entrenamiento y bienestar) · EP01 JVY0101

Este documento justifica la existencia de **ms-rutinas** como microservicio independiente: qué requisitos del negocio cubre (funcionales, no funcionales y de seguridad), por qué está delimitado así (SRP), y qué tecnología AWS se usa para cada responsabilidad y **por qué**. Los diagramas que respaldan esta justificación están en `docs/diagramas/`.

---

## 1. Misión del servicio

ms-rutinas mantiente el seguimiento en tiempo real del proceso: consume los eventos y señales del dominio (estados, posición, sensores, etapas), mantiene el estado vigente consultable en milisegundos y notifica las transiciones del caso caso15 (FitLife).
...
| RF | Requisito (de `00_PresentacionEmpresa.md`) | Qué hace ms-rutinas al respecto | Evidencia |
|----|------------------------------------------|-------------------------------|-----------|
| **RF-02** | Crear y consultar rutinas de entrenamiento planeadas | Ingresa y consulta el estado en tiempo real: cada evento actualiza el recurso y dispara la notificación correspondiente | diagrama de secuencia (evento en tiempo real) |
...
| RNF | Criterio | Cómo lo cumple ms-rutinas | Decisión técnica |
|-----|----------|--------------------------|------------------|
| **RNF-05** (Rendimiento) | Disponibilidad reflejada en menos de 5 segundos; confirmación en menos de 3 segundos | Respuestas dentro del umbral exigido por el caso (ver alarmas p95) | Caché/bajo acoplamiento + CloudWatch con alarma de latencia |
| **RNF-01** (Escalabilidad) | Ingerir miles de eventos por segundo sin pérdida | Auto scaling independiente de este servicio (2→20 tareas Fargate según carga) | ECS Fargate + alarmas de CloudWatch: solo este componente escala en el pico |
| **RNF-06** (Consistencia) | Ningún evento debe perderse (colas persistentes) | Escritura persistida antes de confirmar; eventos por cola con DLQ ante fallas | Aurora transaccional + SQS persistido + idempotencia |

**Justificación SRP (IE9):** ms-rutinas tiene **una sola razón de cambio**: las reglas de seguimiento y transición de estados del proceso en tiempo real. Si mañana cambia esa regla, **ningún otro servicio se modifica**.
...
| Tecnología | Para qué se usa en ms-rutinas |
|------------|------------------------------|
...
| Servicio AWS | Rol en ms-rutinas | Por qué se eligió |
|--------------|----------------|--------------------|
...
## 6. Delimitación: qué NO hace ms-rutinas (IE9/IE10)
...
| planes | ms-planes | razones de cambio distintas: la operación se orquesta aquí, pero cada colaborador es autónomo |
...
├── secuencia/
│   └── Secuencia-Rutina   evento en tiempo real
└── infraestructura/
